...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  loading$ = combineLatest([
    ...
    map(
      ([parentLoading, childLoading, adminLoading]) =>
        parentLoading || childLoading || adminLoading
    )
  );
  ...
  ngOnInit() {
    ...
    this.tasks$.pipe(takeUntil(this.destroy$)).subscribe((tasks) => {
      if (tasks) {
        ...
        this.pendingTaskCount = tasks.filter(
    ...
    combineLatest([
      this.parents$,
      this.adminChildren$,
      this.store.select(AdminSelectors.selectAdmins),
    ])
      .pipe(takeUntil(this.destroy$))
      .subscribe(([parents, children, admins]) => {
  ...
  private loadAdminData() {
    ...
    // Generate random activity data for now (in a real app, this would come from the API)
      ...
      datasets: [
        {
          ...
          data: [parentsCount, childrenCount, adminsCount],
        },
      ],
    };
  }
  ...
  private generateRandomActivityData() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      this.recentActivities.push({
        ...activity,
        description: `${activity.user} ${this.getActivityDescription(
          activity.type
        )}`,
  ...
  private updateAchievements(points: number, tasks: Task[]) {
    ...
    this.achievements[0].unlocked = tasks.some(
      (t) => t.status === TaskStatus.COMPLETED
    );

    // 5-Day streak (placeholder logic - in a real app this would be more complex)
    this.achievements[1].unlocked =
      tasks.filter((t) => t.status === TaskStatus.COMPLETED).length >= 5;
