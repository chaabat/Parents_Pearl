...
export class ParentService {
  ...
  updateParentProfile(parentId: number, profileData: Partial<Parent>): Observable<Parent> {
    ...
    return this.http
      .put<Parent>(`${this.apiUrl}/parents/${parentId}`, profileData, { headers: this.getAuthHeaders() })
      .pipe(
  ...
  searchTasks(parentId: number, keyword: string): Observable<Task[]> {
    return this.http.get<Task[]>(`${this.apiUrl}/parents/${parentId}/tasks/search?keyword=${keyword}`, {
      headers: this.getAuthHeaders(),
    })
  }
