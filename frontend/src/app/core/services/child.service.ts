...
export class ChildService {
  ...
  getMyTasks(childId: number): Observable<Task[]> {
    return this.http.get<Task[]>(`${this.apiUrl}/children/${childId}/my-tasks`, { headers: this.getAuthHeaders() })
  }
  ...
  viewMyTask(childId: number, taskId: number): Observable<Task> {
    return this.http.get<Task>(`${this.apiUrl}/children/${childId}/tasks/${taskId}`, { headers: this.getAuthHeaders() })
  }

  searchMyTasks(childId: number, keyword: string): Observable<Task[]> {
    return this.http.get<Task[]>(`${this.apiUrl}/children/${childId}/tasks/search`, {
      params: { keyword },
      headers: this.getAuthHeaders(),
    })
  }
  ...
  submitTaskAnswer(childId: number, taskId: number, answer: string): Observable<TaskAnswer> {
    return this.http.post<TaskAnswer>(`${this.apiUrl}/children/${childId}/tasks/${taskId}/submit`, answer, {
      headers: this.getAuthHeaders(),
    })
  }
  ...
  getMyTotalPoints(childId: number): Observable<number> {
    return this.http.get<number>(`${this.apiUrl}/children/${childId}/points/total`, { headers: this.getAuthHeaders() })
  }
  ...
  // Reward endpoints
  redeemReward(childId: number, rewardId: number): Observable<RewardRedemption> {
    ...
    return this.http.post<RewardRedemption>(url, { rewardId }, { headers: this.getAuthHeaders() }).pipe(
      tap((response) => {
  ...
  getAvailableRewards(childId: number): Observable<Reward[]> {
    return this.http.get<Reward[]>(`${this.apiUrl}/children/${childId}/rewards`, { headers: this.getAuthHeaders() })
  }

  getRedemptionHistory(childId: number): Observable<RewardRedemption[]> {
    return this.http.get<RewardRedemption[]>(`${this.apiUrl}/children/${childId}/rewards/history`, {
      headers: this.getAuthHeaders(),
    })
  }
  ...
  getChildRedemptions(childId: number): Observable<RewardRedemption[]> {
    return this.http
      .get<RewardRedemption[]>(`${this.apiUrl}/children/${childId}/rewards/history`, { headers: this.getAuthHeaders() })
      .pipe(tap((response) => console.log("Redemption history response:", response)))
  }
