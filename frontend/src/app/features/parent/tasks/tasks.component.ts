...
import { debounceTime, distinctUntilChanged, take, map, takeUntil, finalize } from "rxjs/operators"
...
export class TasksComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: string[] = ["title", "description", "pointValue", "taskType", "status", "dueDate", "actions"]
  ...
  ngOnInit(): void {
    ...
    // Updated search subscription with direct service call and fallback
    this.searchControl.valueChanges
      ...
      .subscribe((keyword) => {
        if (!keyword || keyword.length < 2) {
          ...
          // Use direct service call instead of store action
          this.searching = true
          this.searchError = null
          ...
          if (this.parentId) {
            ...
            this.parentService
              .searchTasks(this.parentId, keyword)
              .pipe(
                finalize(() => {
                  this.searching = false
                }),
              ...
              .subscribe({
                ...
                error: (error) => {
                  // This should not happen now since we're using client-side search directly
                  console.error("Search error in component (should not happen):", error)
                  this.searchError = "Using local filtering for search results."
                },
              })
          }
        }
  ...
  private formatTaskData(formValue: any): Partial<Task> {
    ...
    return {
      ...formValue,
      childId,
      dueDate: formValue.dueDate instanceof Date ? formValue.dueDate.toISOString().split("T")[0] : formValue.dueDate, // Keep original value if not a Date object
      choices:
  ...
  onChildChange(childId: number) {
    ...
    this.store.select(ParentSelectors.selectChildById(childId)).subscribe((child) => {
      if (child) {
