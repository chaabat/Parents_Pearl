...
export class ChildrenComponent implements OnInit, OnDestroy {
  ...
  childStats: { [childId: number]: { taskCount: number; rewardCount: number } } = {}
  ...
  constructor(
    ...
    this.pointsForm = this.fb.group({
      points: [0, [Validators.required, Validators.min(-100), Validators.max(100)]],
    ...
    if (childId !== undefined) {
      ...
      if (this.parentId) {
        ...
        forkJoin({
          ...
          pointHistory: this.parentService.getChildPointHistory(this.parentId, childId),
        })
          .pipe(takeUntil(this.destroy$))
          .subscribe({
            next: (results) => {
              // Count tasks assigned to this child
              this.childStats[childId].taskCount = results.tasks.filter((task) => task.childId === childId).length

              // Count reward redemptions (points with negative values are reward redemptions)
              this.childStats[childId].rewardCount = results.pointHistory.filter((point) => point.points < 0).length
            },
            error: (error) => {
  ...
  uploadImage(): Observable<string> {
    ...
    return this.http
      .post<{ imageUrl: string }>(`${environment.apiUrl}/uploads/images`, formData)
  ...
  getImageUrl(picture: string | undefined | null): string {
    ...
    return picture.startsWith("http") ? picture : `https://res.cloudinary.com/dlwyetxjd/image/upload/${picture}`
  }
